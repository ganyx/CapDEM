...
#pragma omp parallel for num_threads(NTHREADS)	// YG, MPI	
	for( int ip=0;ip<P.size();ip++){
		...
		if(P[ip].void_volume <= 1e-2 * P[ip].grain_volume) 
			P[ip].void_volume = 1e-2 * P[ip].grain_volume; // avoid negative Void volume
		P[ip].saturation = P[ip].water_volume/P[ip].void_volume; // update the saturation of each cell
		if(P[ip].saturation > MAX_SATURATION) P[ip].saturation = MAX_SATURATION; //exp
		}
...
#pragma omp parallel for num_threads(NTHREADS)	// YG, MPI
	...
	for(int ic=0;ic<C.size();ic++){
		if(C[ic].water_volume > 0.0) // if liquid bridge is existing
		{
			...
			double Height = 100.0;
			if(fabs(C[ic].pA->X.x[0] - C[ic].pB->X.x[0])<10.0){
				pa_ave = C[ic].pA->water_pressure + cell.normal_stress_ext/Height * C[ic].pA->X.x[0];
				pb_ave = C[ic].pB->water_pressure + cell.normal_stress_ext/Height * C[ic].pB->X.x[0];
			}
			else{ // no water tranport cross X.x[0] boundary
				pa_ave = 0.0;
				pb_ave = 0.0;
			}
...
//			double area = min(pow(C[ic].water_volume, 2.0/3.0),C[ic].voronoi_area);
			...
			C[ic].dot_water_volume = - area * parameter.LIQUID_DIFFUSION * 
			...
			// lower bond for the water content
			double vwmin=0.01;
			if(C[ic].pA->water_volume < vwmin){
				C[ic].pB->water_volume += C[ic].pA->water_volume-vwmin;
				C[ic].pA->water_volume = vwmin;
			}
			if(C[ic].pB->water_volume < vwmin){
				C[ic].pA->water_volume += C[ic].pB->water_volume-vwmin;
				C[ic].pB->water_volume = vwmin;
			}
		}
	...
	for(int ic=0;ic<C.size();ic++){
		...
		// Water re-distribution through contacts. TO-DO!
//		C[ic].pA->water_volume += dV/2.0 - dV_pA; //exp
//		C[ic].pB->water_volume += dV/2.0 - dV_pB; //exp
	}
